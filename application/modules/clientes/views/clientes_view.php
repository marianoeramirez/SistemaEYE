...
$("#list").jqGrid({
	...
	datatype: "json",
	mtype: 'post',
   	colNames:['Cedula','User', 'Nombres','Apellidos','Email','Sexo','Telf Local','Telf Celular','Nacimiento','Tipo','Password'],
   	colModel:[
   		...
   		{name:'usuario',index:'usuario', width:80, editable:true, editrules:{required:true}},
   		...
   		{name:'tipo',index:'id_tipo',  width:90, "stype":"select","searchoptions":{"value":<?php echo json_encode($tipo); ?>}, editable:true,edittype:"select" ,editoptions:{value:<?php echo json_encode(array_filter($tipo)); ?>},editrules:{ required:true}},
...
}).jqGrid("navGrid", "#pager",{edit:true,add:true,del:true,view:true,search:false,excel:false,addtitle:'Agregar Usuarios'}, {resize:false, editCaption:'Modificar Usuarios'}, {resize:false,addCaption:'Agregar Usuarios'}, {},{}, {resize:false,caption:'Consultar Usuarios'})
.jqGrid("filterToolbar");
$('#btncon').click(function(){
	var gr = $("#list").jqGrid('getGridParam','selrow');
	if( gr != null ) $("#list").jqGrid('viewGridRow',gr,{height:350,resize:false, caption:'Consultar Usuarios'});
	else alert("Por favor seleccina una fila");
}).button({icons: {primary: "ui-icon-document"}});
$('#btnadd').click(function(){
	$("#list").jqGrid('editGridRow',"new",{height:350,reloadAfterSubmit:true,resize:false,addCaption:'Agregar Usuarios'});
}).button({icons: {primary: "ui-icon-plus"}});
$("#btnmod").click(function(){
	var gr = $("#list").jqGrid('getGridParam','selrow');
	if( gr != null ) $("#list").jqGrid('editGridRow',gr,{height:350,reloadAfterSubmit:false,resize:false, editCaption:'Modificar Usuarios'});
