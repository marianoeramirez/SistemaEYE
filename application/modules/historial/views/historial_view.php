...
function range_value(elem, operation, value){
	if(operation === 'get') {
       return elem.val();
    } else if(operation === 'set') {
       $('input',elem).val(value);
	   $(elem).data("rangeinput").setValue(value);
    }
}
function prueba(){
	alert("hola");
...
$("#list").jqGrid({
	...
	datatype: "json",
	mtype: 'post',
   	colNames:['Código','Cliente','Fecha','Astigmatismo OD','Astigmatismo OI','Hipermetropia OD','Hipermetropia OI','Presbicia OD','Presbicia OI','Miopia OD','Miopia OI','Observaciones'],
   	colModel:[
   		...
   		{name:'info',index:'info', width:80, editable:true, edittype:"textarea", editrules:{required:false, edithidden:true}, hidden:true},
   	],
...
}).jqGrid("navGrid", "#pager",{edit:true,add:true,del:false,view:true,search:false,excel:false,addtitle:'Agregar Historial'}, {height:500,resize:true, editCaption:'Modificar Historial', width:400}, {height:500,width: 400,resize:true,addCaption:'Agregar Historial'}, {},{}, {height:500,width: 400,resize:false,caption:'Consultar Historial'})
.jqGrid("filterToolbar").bind('jqGridAddEditBeforeShowForm',function(){
	$('.rango').rangeinput();
});
$('#btncon').click(function(){
	var gr = $("#list").jqGrid('getGridParam','selrow');
	if( gr != null ) $("#list").jqGrid('viewGridRow',gr,{height:500, width: 400, resize:false, caption:'Consultar Historial'});
	else alert("Por favor seleccina una fila");
}).button({icons: {primary: "ui-icon-document"}});
$('#btnadd').click(function(){
	$("#list").jqGrid('editGridRow',"new",{height:500,width: 400,reloadAfterSubmit:true,resize:false,addCaption:'Agregar Historial'});

}).button({icons: {primary: "ui-icon-plus"}});
$("#btnmod").click(function(){
	var gr = $("#list").jqGrid('getGridParam','selrow');
	if( gr != null ) $("#list").jqGrid('editGridRow',gr,{height:500,width: 400,reloadAfterSubmit:false,resize:false, editCaption:'Modificar Historial'});
